...
def test_validate_raises_when_required_vars_missing(monkeypatch: pytest.MonkeyPatch):
...
def test_load_refreshes_module_globals(monkeypatch: pytest.MonkeyPatch, env_paths: tuple[Path, Path]):
...
def test_load_optional_int_defaults(monkeypatch: pytest.MonkeyPatch):
...
def test_load_optional_int_override(monkeypatch: pytest.MonkeyPatch):
...
def test_load_https_only_true(monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setenv("HTTPS_ONLY", "true")
    env.load()
    assert env.HTTPS_ONLY is True


def test_load_https_only_false(monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setenv("HTTPS_ONLY", "false")
    env.load()
    assert env.HTTPS_ONLY is False


def test_load_router_creds_empty_string_becomes_none(monkeypatch: pytest.MonkeyPatch):
...
def test_load_vapid_keys(monkeypatch: pytest.MonkeyPatch):
