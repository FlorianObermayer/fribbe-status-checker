...
from app.services.persistent_collections import (
    ConversionHelper,
    DictSerializable,
...
def test_persistentdict_write_to_non_existing_subfolder() -> None:
...
def test_persistentdict_write_and_read() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
...
def test_persistentdict_delete() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
        path = str(Path(tmpdir) / "test.json")
...
def test_persistentdict_len_and_iter() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
        path = str(Path(tmpdir) / "test.json")
        d: PersistentDict[int] = PersistentDict(path, int)
        d["a"] = 1
...
class MyClass(DictSerializable):
    ...
    @classmethod
    def from_dict(cls, d: dict[str, Any]) -> "MyClass":
...
class MyDataClass(DictSerializable):
    ...
    @classmethod
    def from_dict(cls, d: dict[str, Any]) -> "MyDataClass":
        return cls(**d)


def test_persistentdict_with_class() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
...
def test_persistentdict_with_class_in_list() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
...
def test_persistentdict_with_dataclass() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
...
def test_persistentdict_with_dataclass_omitting_protocol_expect_type_error() -> None:
...
class NestedClass(DictSerializable):
    ...
    @classmethod
    def from_dict(cls, d: dict[str, Any]) -> "NestedClass":
        child = cls.from_dict(d["child"]) if d["child"] else None
        return cls(d["name"], d["values"], child)


def test_persistentdict_nested_dictserializable() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert d2["n"].child.name == "child"  # pyright: ignore[reportOptionalMemberAccess]
        assert d2["n"].child.values == [3, 4]  # pyright: ignore[reportOptionalMemberAccess]
        assert d2["n"].child.child is None  # pyright: ignore[reportOptionalMemberAccess]


def test_persistentdict_nested_dict() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
...
def test_persistentdict_nested_dict_any_type_not_supported() -> None:
    with pytest.raises(TypeError):
        _ = PersistentDict[dict[str, Any]]("my_path", value_type=dict[str, Any])


def test_persistentdict_nested_list_any_type_not_supported() -> None:
    with pytest.raises(TypeError):
        _ = PersistentDict[list[list[list[Any]]]]("my_path", value_type=list[list[list[Any]]])


def test_persistentdict_nested_list() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
...
def test_persistentlist_with_union_types_not_supported() -> None:
    with pytest.raises(TypeError):
        _ = PersistentDict("path", value_type=list[int | list[int]])


def test_persistentlist_with_int() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
...
def test_persistentlist_with_class() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
        path = str(Path(tmpdir) / "list.json")
...
def test_persistentlist_with_dataclass() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
        path = str(Path(tmpdir) / "list.json")
...
def test_persistentdict_with_datetime() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
...
def test_persistentdict_with_naive_datetime() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
...
def test_persistentlist_with_datetime() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
...
def test_persistentdict_with_mixed_datetime_formats() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
...
def test_persistentobject_with_int() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
...
def test_persistentobject_with_class() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        obj.set(test_obj)
        assert obj.get().foo == "test"  # pyright: ignore[reportOptionalMemberAccess]
        assert obj.get().bar == 123  # pyright: ignore[reportOptionalMemberAccess]

        # Reload from file
        obj2 = PersistentObject(path, MyClass)
        assert obj2.get().foo == "test"  # pyright: ignore[reportOptionalMemberAccess]
        assert obj2.get().bar == 123  # pyright: ignore[reportOptionalMemberAccess]


def test_persistentobject_with_dataclass() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Test default value
        assert obj.get().foo == "default"  # pyright: ignore[reportOptionalMemberAccess]
        assert obj.get().bar == 0  # pyright: ignore[reportOptionalMemberAccess]
        ...
        assert obj2.get().foo == "new"  # pyright: ignore[reportOptionalMemberAccess]
        assert obj2.get().bar == 42  # pyright: ignore[reportOptionalMemberAccess]


def test_persistentobject_with_datetime() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
...
def test_persistent_decorator() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
...
def test_atomic_write_leaves_valid_file_on_success() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
...
def test_batch_write_saves_once() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
...
def test_batch_write_delete() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
...
def test_batch_write_clear() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
...
def test_batch_write_exception_still_saves() -> None:
    """On exception the batch should still save whatever mutations happened."""
    with tempfile.TemporaryDirectory() as tmpdir:
        path = str(Path(tmpdir) / "batch_exc.json")
        d: PersistentDict[int] = PersistentDict(path, int)
        with pytest.raises(RuntimeError), d.batch_write() as store:  # noqa: PT012
            store["a"] = 1
...
def test_batch_write_read_within_batch() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
...
def test_batch_write_other_thread_still_locks() -> None:
...
def test_batch_write_concurrent_no_lost_lock() -> None:
...
def test_batch_write_nested_same_thread() -> None:
...
def test_list_to_range_single_element() -> None:
    r = ConversionHelper.list_to_range([5])
    assert r == range(5, 6)


def test_list_to_range_even_step() -> None:
    r = ConversionHelper.list_to_range([2, 4, 6, 8])
    assert r == range(2, 10, 2)


def test_list_to_range_uneven_raises() -> None:
...
def test_persistentdict_with_range() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
...
def test_persistentdict_with_float() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
...
def test_persistentdict_with_bool() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
...
def test_persistentdict_with_tuple() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
...
def test_persistentdict_with_variadic_tuple() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
...
def test_persistentdict_with_set() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
...
def test_persistentdict_values_and_items() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
...
def test_persistentdict_contains() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
...
def test_persistentdict_clear_outside_batch() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
...
def test_persistentdict_reload() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
...
def test_persistentlist_extend() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
...
def test_persistentlist_setitem_and_delitem() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
...
def test_persistentlist_to_list() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
...
def test_persistent_descriptor_class_access_returns_default() -> None:
...
def test_persistent_descriptor_set_updates_storage() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
        config = DecoratedConfig(tmpdir)
        config.name = "updated"
        config.count = 42
...
def test_persistent_descriptor_raises_on_non_provider() -> None:
