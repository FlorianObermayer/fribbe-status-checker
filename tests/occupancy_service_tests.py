...
def service() -> OccupancyService:
    return OccupancyService()


@pytest.mark.asyncio
@patch("app.services.occupancy.occupancy_service.OccupancyService._get_occupancy_data")
async def test_run_get_latest_occupancy_sets_week_occupancy(
    ...
    await s._run_get_latest_occupancy()
    assert hasattr(s, "_week_occupancy")
    assert len(s._week_occupancy) > 0


def test_get_todays_occupancy_no_occupancy() -> None:
    s = service()
