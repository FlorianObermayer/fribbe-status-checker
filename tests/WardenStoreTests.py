...
def test_get_all_empty(store: WardenStore):
    assert store.get_all() == []


def test_get_all_returns_copy(populated_store: WardenStore):
...
def test_add_new_warden(store: WardenStore):
    store.add(Warden("Alice"))
    assert len(store.get_all()) == 1
    assert store.get_all()[0].name == "Alice"


def test_add_duplicate_name_raises(store: WardenStore):
    store.add(Warden("Alice"))
    with pytest.raises(ValueError, match="already exists"):
        store.add(Warden("Alice"))


def test_add_duplicate_name_case_insensitive(store: WardenStore):
    store.add(Warden("Alice"))
    with pytest.raises(ValueError, match="already exists"):
        store.add(Warden("alice"))


def test_add_normalises_macs_to_lowercase(store: WardenStore):
...
def test_by_name_found(populated_store: WardenStore):
    warden = populated_store.by_name("Alice")
    assert warden.name == "Alice"


def test_by_name_case_insensitive(populated_store: WardenStore):
    warden = populated_store.by_name("alice")
    assert warden.name == "Alice"


def test_by_name_not_found_raises(store: WardenStore):
...
def test_first_or_none_match_by_mac(populated_store: WardenStore):
    result = populated_store.first_or_none("aa:bb:cc:dd:ee:ff", None)
    assert result is not None
    assert result.name == "Alice"


def test_first_or_none_match_by_mac_case_insensitive(populated_store: WardenStore):
    result = populated_store.first_or_none("AA:BB:CC:DD:EE:FF", None)
    assert result is not None
    assert result.name == "Alice"


def test_first_or_none_match_by_device_name(populated_store: WardenStore):
    result = populated_store.first_or_none(None, "alices-macbook")
    assert result is not None
    assert result.name == "Alice"


def test_first_or_none_no_match(populated_store: WardenStore):
    assert populated_store.first_or_none("ff:ff:ff:ff:ff:ff", None) is None


def test_first_or_none_both_none(populated_store: WardenStore):
...
def test_update_warden(populated_store: WardenStore):
...
def test_update_not_found_raises(store: WardenStore):
    with pytest.raises(ValueError, match="No warden found"):
        store.update("nobody", Warden("nobody"))


def test_update_case_insensitive(populated_store: WardenStore):
    updated = Warden("Alice", ["99:88:77:66:55:44"])
    populated_store.update("alice", updated)
...
def test_delete_warden(populated_store: WardenStore):
...
def test_delete_not_found_raises(store: WardenStore):
    with pytest.raises(ValueError, match="No warden found"):
        store.delete("nobody")


def test_delete_case_insensitive(populated_store: WardenStore):
    populated_store.delete("alice")
    with pytest.raises(ValueError):
        populated_store.by_name("Alice")
