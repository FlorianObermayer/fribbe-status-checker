...
def test_multiple_start_calls_ignored() -> None:
    svc = _CountingService()
    svc.start_polling(interval=60)
    svc.wait_for_poll()
...
def test_poll_runs_multiple_times() -> None:
    svc = _CountingService()
...
def test_delay_defers_first_poll() -> None:
    svc = _CountingService()
...
def test_exception_in_poll_does_not_kill_loop() -> None:
    svc = _FailingService()
    svc.start_polling(interval=0)
    svc.wait_for_poll()
    svc.wait_for_poll()
...
def test_stop_event_interrupts_sleep() -> None:
    svc = _CountingService()
