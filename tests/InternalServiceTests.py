...
def test_first_device_on_site_set_when_none_and_new_devices(service: InternalService):
    service._first_device_on_site = None  # type: ignore
    ...
    assert service._first_device_on_site is not None  # type: ignore


def test_first_device_on_site_not_set_when_already_set(service: InternalService):
    now = datetime.now(tz=ZoneInfo("Europe/Berlin"))
    service._first_device_on_site = now  # type: ignore
    service._active_devices_ct = 0  # type: ignore
    service._last_updated = now  # type: ignore
    service._update_device_statistics(2)  # type: ignore
    assert service._first_device_on_site == now  # type: ignore
...
def test_last_device_on_site_set_when_transition_to_zero(service: InternalService):
    ...
    service._last_device_on_site = None  # type: ignore
    service._active_devices_ct = 2  # type: ignore
    service._last_updated = datetime.now(tz=ZoneInfo("Europe/Berlin"))  # type: ignore
    service._update_device_statistics(0)  # type: ignore
    assert service._last_device_on_site is not None  # type: ignore
...
def test_reset_at_5am(service: InternalService):
    ...
    assert service._last_device_on_site is None  # type: ignore
...
def test_no_reset_if_same_virtual_day(service: InternalService):
    ...
    service._first_device_on_site = now  # type: ignore
    service._last_device_on_site = now  # type: ignore
    service._active_devices_ct = 0  # type: ignore
    service._update_device_statistics(0)  # type: ignore
    assert service._first_device_on_site == now  # type: ignore
    assert service._last_device_on_site == now  # type: ignore
